...
class TensorIterator : public thrust::iterator_adaptor<TensorIterator<Iterator, N>, Iterator> {
   public:
     ...
     */
    __host__ __device__ TensorIterator(const Iterator& iter, const size_t* shape, const size_t* strides, const size_t start, const Order order, const bool contiguous)
