...
class Tensor {
   public:
     ...
     * @param shape Shape of the tensor.
     */
    Tensor(const Shape<N>& shape, const Order order) : _shape(shape), _size(get_size(shape)), _strides(get_strides(shape, order)), _start(0), _contiguous(true), _order(order) {
    ...
    TensorCondProxy<T, N> operator[](const Tensor<bool, N>& cond) const {
        assert_at_least_1d_tensor<N>();
        assert_same_order_between_two_tensors(_order, cond.order());
