...
TEST(TensorConstructor, BasicAssertions) {
    ...
    // Test for F order
    auto shape4 = vt::Shape<3>{1, 2, 3};
    auto tensor4 = vt::Tensor<float, 3>(shape4, vt::Order::F);
    EXPECT_EQ(tensor4.size(), 6);
    EXPECT_EQ(tensor4.shape(), (vt::Shape<3>{1, 2, 3}));
    EXPECT_EQ(tensor4.strides(), (vt::Shape<3>{1, 1, 2}));
    EXPECT_EQ(tensor4.start(), 0);
    EXPECT_EQ(tensor4.order(), vt::Order::F);
    EXPECT_EQ(tensor4.contiguous(), true);
    EXPECT_EQ(vt::asvector(tensor), (std::vector<float>{0, 0, 0, 0, 0, 0}));
...
TEST(TensorReshape, BasicAssertions) {
    ...
    // Test for F order
    auto tensor2 = vt::arange(6, vt::Order::F).reshape(1, 2, 3);
    EXPECT_EQ(tensor2.size(), 6);
    EXPECT_EQ(tensor2.shape(), (vt::Shape<3>{1, 2, 3}));
    EXPECT_EQ(tensor2.strides(), (vt::Shape<3>{1, 1, 2}));
...
TEST(SlicedTensorReshape, BasicAssertions) {
    ...
    // Test for F order
    auto tensor2 = vt::arange(12, vt::Order::F)({0, 12, 2});
    auto tensor3 = tensor2.reshape(1, 2, 3);
    EXPECT_EQ(tensor2.size(), 6);
    EXPECT_EQ(tensor3.shape(), (vt::Shape<3>{1, 2, 3}));
    EXPECT_EQ(tensor3.strides(), (vt::Shape<3>{1, 1, 2}));
    EXPECT_EQ(tensor3.start(), 0);
    EXPECT_EQ(tensor3.order(), vt::Order::F);
    EXPECT_EQ(tensor3.contiguous(), true);
    EXPECT_EQ(vt::asvector(tensor3), (std::vector<float>{0, 2, 4, 6, 8, 10}));
}
...
TEST(TensorInPlacePlusScalar, BasicAssertions) {
    ...
    // Test for F order
    auto tensor2 = vt::arange(6, vt::Order::F);
    tensor2 += 1;
    EXPECT_EQ(vt::asvector(tensor2), (std::vector<float>{1, 2, 3, 4, 5, 6}));
}
...
TEST(TensorInPlaceMinusScalar, BasicAssertions) {
    ...
    auto tensor2 = vt::arange(6, vt::Order::F);
    tensor2 -= 1;
    EXPECT_EQ(vt::asvector(tensor2), (std::vector<float>{-1, 0, 1, 2, 3, 4}));
}
...
TEST(TensorInPlaceDivideScalar, BasicAssertions) {
    ...
    // Test for F order
    auto tensor2 = vt::arange(6, vt::Order::F) * 2.0f;
    tensor2 /= 2.0f;
    EXPECT_EQ(vt::asvector(tensor2), (std::vector<float>{0, 1, 2, 3, 4, 5}));
}
...
TEST(TensorInPlaceMultiplyTensor, BasicAssertions) {
    ...
    auto tensor4 = vt::arange(6, vt::Order::F);
    tensor3 *= tensor4;
    EXPECT_EQ(vt::asvector(tensor3), (std::vector<float>{0, 1, 4, 9, 16, 25}));
    EXPECT_EQ(vt::asvector(tensor4), (std::vector<float>{0, 1, 2, 3, 4, 5}));
}
...
TEST(TensorInPlaceDivideTensor, BasicAssertions) {
    ...
    // Test for F order
    auto tensor3 = (vt::arange(6, vt::Order::F) + 1.0f) * 2.0f;
    auto tensor4 = vt::arange(6, vt::Order::F) + 1.0f;
    tensor3 /= tensor4;
    EXPECT_EQ(vt::asvector(tensor3), (std::vector<float>{2, 2, 2, 2, 2, 2}));
    EXPECT_EQ(vt::asvector(tensor4), (std::vector<float>{1, 2, 3, 4, 5, 6}));
}
